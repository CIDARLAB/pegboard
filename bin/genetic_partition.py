...
Written by Jing Zhang <jgzhang@bu.edu>, W.R. Jackson <wrjackso@bu.edu>
Densmore Lab/DAMP Lab, Boston University
All Rights Reserved
...
def part_sol_filename(settings, sample):
    return settings[sample]['output_path'] + '/DAG' + settings[sample]['n'] + '.' + settings[sample][
        'k'] + '/part_solutions.txt'
...
def opt_gate_assignment_filename(settings, sample, run):
    return settings[sample]['output_path'] + '/DAG' + settings[sample]['n'] + '.' + settings[sample][
        'k'] + '/optimal gate assignments_trajectory_' + str(run) + '.txt'
...
def load_gate_assignment(settings, sample, G):
    ...
    for line in lines[1:]:
        ...
        if G.nodes[node]['type'] == 'primitive':
            G.nodes[node]['weight'] = gateLib[G.nodes[node]['gate']]['toxicity']
        else:
            G.nodes[node]['weight'] = 0
    return G


def load_partition(settings, sample):
    lines = [open(part_sol_filename(settings, sample), 'r').read().strip("\n")][0].split('\n')
    cut = lines[0].split('\t')[1]
    part_dict = dict()
    for line in lines[1:]:
        tokens = line.split('\t')
        part = int(tokens[0].split(' ')[1])
        nodes = [int(n) for n in tokens[1].split(',')]
        for n in nodes:
            part_dict[n] = part
    return (int(cut), [part_dict[n] for n in sorted(part_dict.keys())])
...
def calc_circuit_score(settings, sample, assignedLib, gateLib, sensorLib):
...
def assign_gates(settings, sample, G, edge_direct, partition):
    ...
    for i in range(0, max(partition[1]) + 1):
        ...
        for idx, v in enumerate(gnodes):
            vg = chosengates[idx]
            vg_rbs = [key for key in gateLib if
                      key.split('-')[0] == str(vg)]  # different rbs choices for this repressor gate
            vg = random.choice(vg_rbs)
            assignedLib[v]['gate'] = vg
            assignedLib[v]['params'] = [float(gateLib[vg]['ymin']), float(gateLib[vg]['ymax']), float(gateLib[vg]['K']),
                                        float(gateLib[vg]['n'])]
            assignedLib[v]['type'] = 'primitive'

    # calculate circuit score of initial assignments
    S0, circuit_valid = calc_circuit_score(settings, sample, assignedLib, gateLib, sensorLib)

    # simulated annealing
    Tmax = 1000  # starting temperature
    C = 1e-5

    if circuit_valid:
        for i in range(5):  # for x number of trajectories
            SList = []
            Smax = S0
            ...
            for t in range(1000000):  # perform simulated annealing
                ...
                if g_swap_t in used_gates:  # if swapping two gates within the same partition
                    ...
                    assignedLib_tmp[g_swap_f]['params'] = [float(gateLib[assignedLib_tmp[g_swap]['gate']]['ymin']),
                    ...
                    assignedLib_tmp[g_swap]['params'] = [float(gateLib[g_swap_t]['ymin']),
                                                         float(gateLib[g_swap_t]['ymax']),
                                                         float(gateLib[g_swap_t]['K']), float(gateLib[g_swap_t]['n'])]
                ...
                S, circuit_valid = calc_circuit_score(settings, sample, assignedLib_tmp, gateLib, sensorLib)
...
def create_temp_part(G, nodes_dict, outnodes, blockbs):
    ...
    block_dupgate = [i for i in block_gates.keys() if
                     len(block_gates[i]) != len(set(block_gates[i]))]  # list of blocks that have dupliate gates
...
def _move_nodes(G_primitive, owblock, uwblock, block_load):
    ...
    for outnodes in outList:
        ...
        blockbList = [p for p in
                      itertools.product(uwblock, repeat=len(outnodes))]  # choose target block to move these nodes to
        ...
        for blockbs in blockbList:
            ...
            part_temp, new_load = create_temp_part(G_primitive, nodes_dict, outnodes, blockbs)
            if part_temp != False:
                # if new partition is OK (all blocks have no duplicated gates nor being over weight limit),
                # calculate the change in cut size
                cut = cal_cut(G_primitive, part_temp)
                # store delta cut size in solutions
                gates = [G_primitive.nodes[v]['gate'] for v in G_primitive.nodes()]
                solns[idx] = {'original partition': part_opt[1], 'loads': block_load.values(),
                              'new partition': list(part_temp.values()), \
                              'gates': gates, 'new loads': new_load.values(), 'new cut': cut,
                              'original cut': part_opt[0]}
                idx += 1
    return solns


def refine_blocks(settings, sample, ow):
    ...
    part_opt = load_partition(settings, sample)
    ...
    # for the optimal partition that yields the minimum cuts, calculate the average weights in each block
    block_load, block_gates = obt_block_feat(G_primitive)
    block_ow = [i for i in block_load if block_load[i] > ow]  # list of blocks that are overweight
    ...
    block_uw = [i for i in block_uw if block_load[i] < ow - 0.94]
    gates = [G_primitive.nodes[v]['gate'] for v in G_primitive.nodes()]
    loads = block_load.values()

    if len(block_ow) in [1, 2, 3]:
        # print('overloaded blocks', block_ow)
        solns = _move_nodes(G_primitive, part_opt, block_ow, block_uw, block_load)
        if solns == {}:
            solns = {
                -1: {'original partition': part_opt[1], 'original cut': part_opt[0], 'gates': gates, 'loads': loads}}

    else:  # no overweight block or more than 3 overweight blocks
        if len(block_ow) == 0:
            solns = {
                0: {'original partition': part_opt[1], 'original cut': part_opt[0], 'gates': gates, 'loads': loads}}
        else:
            solns = {
                -1: {'original partition': part_opt[1], 'original cut': part_opt[0], 'gates': gates, 'loads': loads}}
...
def record_solns(G, solns, outfile):
    ...
    f_out = open(outfile, 'w')
    f_out.write(
        'Solution\tOri Cut\tOri Load\tOri Partition Result\tGate Assignment\tNew Cut\tNew Load\tNew Partition\n')
    for idx in solns:
        soln = solns[idx]
        if 'new partition' in soln:
            ...
            f_out.write('\t'.join([str(idx), str(soln['original cut']), ','.join([str(i) for i in soln['loads']]), \
                                   ','.join([str(i) for i in soln['original partition']]),
                                   ','.join(soln['gates'])]) + '\n')
